...
Ein weiteres Problem ist die Suche nach einer Partition \(\{S,T\}\) von \(V\) mit \(s \in S\) und \(t \in T\), sowie minimaler \(c(S, T) := \sum\{c_{ij} \mid (i,j) \in E \wedge i \in S \wedge j \in T\}\).

Um das Problem zu lösen, lässt sich der maximale Fluss finden, da \(|f| = c(S,T)\).
Dann kann man jeweils im berechneten Residualgraphen \(G_f\) nach Knoten suchen, die von \(s\) bzw. \(t\) (mit umgedrehten Kanten) erreichbar sind.

\subsubsection{Maximum-Bipartite-Matching}
Flussnetzwerke lassen sich auch verwenden, um das größtmöglichste Matching zwischen Knoten in einem bipartiten Graphen zu finden.
Dabei ist ein Matching eine Teilmenge der Kanten, sodass Knoten nur mittels genau einer Kante verbunden werden können.

Das Problem lässt sich lösen, indem man ein Netzwerk konstruiert, welches eine Quelle hat, die zu allen Knoten aus der ersten Klasse verbunden ist, welche zu den jeweiligen Knoten aus der anderen Klasse verbunden sind, die zur Senke verbunden sind.
Alle Kanten haben Kapazität 1.
Berechnet man nun den maximalen Fluss werden genau die Kanten gefüllt, die in einer maximalen Matching verwendet werden.

\section{Range Queries}
Range Queries beschäftigen sich mit der Frage wie man für \(a \in \N^n\) und \(i,j \in [n], i \le j\), sowie \(f: \N^d \rightarrow \R\) mit \(d \in [n]\) alle möglichen \(f(a[i..j])\) berechnet.
Dabei ist der naive Ansatz \(\O(n)\) langsam, jedoch gibt es Ansätze, um eine Range Query schneller zu bearbeiten.

\subsection{Statische Daten}
Wenn sich das Feld \(a\) nicht verändert, eignen sich folgende Ansätze.
\begin{description}
    \item[Ergebnistabelle] Berechne alle \(n^2\) Einträge \(f(a[i..j])\) vor und speichere sie in einem 2D Array.
        Vorbereitung dauert mindestens \(\Omega(n^2)\), sowie müssen \(\Theta(n^2)\) Werte vorberechnet werden.
        Dafür ist die Auswertung immer \(\O(1)\).

    \item[Präfixsummenarray] Funktioniert nur für \(f = \sum\).
        Speichere ein weiteres Feld \(\sigma \in \N^n\) mit \(\sigma_i = \sum_{j \in [i]}a_j\) für \(i \in [n]\).
        Nun um \(f(a[i..j])\) zu berechnen, gib \(\sigma_j - \sigma_{i-1}\) zurück, wenn \(i > 1\), sonst \(\sigma_j\).
        Die nötige Last sind \(\Theta(n)\) Laufzeit und Speicher für die Vorbereitung, jedoch kann eine Query mit \(\O(1)\) bearbeitet werden.

    \item[Sparse Table]
        Für idempotente Funktionen wie \(\min\) oder \(\max\) eignen sich sog. \emph{Sparse Tables}, bei denen nur für Teilsequenzen mit einer Zweierpotenz als Länge, Werte vorberechnet werden.
        Somit füllt sich eine Tabelle \(M \in \N^{n \times n}\) oder ein Binärbaum mit \(n \log n\) Werten mit \(M[i][i] = a_i\), und \(M[i][i + 2^p - 1] = f(M[i][i + 2^{p-1} - 1], M[i + 2^{p-1}][i + 2^p - 1])\).
        Bei idempotenten Funktionen ist die Auswertung möglich in \(\O(1)\), sonst in \(\O(\log n)\).
\end{description}

\subsection{Dynamische Daten}
Daten können einzeln, \emph{Point Update}, sowie als Teilsequenz, \emph{Range Update}, verändert werden.
Eine Datenstruktur für Range Queries sollten in diesen Fällen nicht von vorne berechnet werden, weswegen sich datenstrukturen anbieten, die Datenveränderungen ermöglichen.
\begin{description}
    \item[Naiver Ansatz] Wenn man keine Hilfsdatenstruktur verwendet, kommt man auf \(\O(n)\) pro Query, jedoch auf \(\O(1)\) pro Update.

    \item[Präfixsummenarray] Beim Präfixsummenarray muss man in einem Update leider \(\O(n)\) Werte verändern, die Query ist aber konstant schnell.
\end{description}
Stattdessen lassen sich Baumstrukturen verwenden, mit denen Query und Update in \(\O(n \log n)\) ermöglicht werden.

\subsubsection{Fenwick-Bäume}
Fenwick-Bäume ermöglichen schnellere Sum Range-Queries auf dynamischen Daten.
In einem Fenwick-Baum speichern Knoten die Gesamtsumme einer gewissen Teilsequenz von \(a\).
Will man nun eine Range-Query \(f(a[i..j])\) berechnen, wählt man Additive und Subtraktive aus dem Baum und erhält das Ergebnis in \(\O(\log n)\).
Das Schöne daran ist, dass man ein Update so implementieren kann, dass das Update vom Kindknoten aus nach Oben backpropagiert wird, und somit auch \(\O(\log n)\) erreicht.
Der Baum kann in \(\O(n)\) erstellt werden, mehr dazu im Competitive Programmer's Handbook: \url{https://cses.fi/book/book.pdf#section.9.2}.

Der Nachteil des Fenwick-Baumes, ist es, dass wie beim Präfixsummenarray, keine anderen Range-Queries unterstützt werden, und, dass Range-Updates nur punktweise funktionieren, man käme also auf \(\O(n \log n)\).

\subsubsection{Segmentbäume}
Um die Probleme von Fenwick-Bäumen zu lösen, kann man Segmentbäume verwenden, die das generelle Prinzip des Fenwick-Baums generalisieren.
Statt genau \(n\) Summen zu speichern, die \([n]\) partitionieren, verwendet man \(2n-1\) Einträge, die jeweils alle möglichen \(f(a[i,i])\), sowie \(f(a[i,i+1])\), \(f(a[i,i+3])\), \(f(a[i,i+7])\), bis \(f(a[i,i+n-1])\) repräsentieren.
Die inneren Knoten speichern also \(f(l, r)\) wobei \(l, r\) die jeweiligen Kinder des Knoten sind.
Um nun \(f(a[i,j])\) zu berechnen, kann der folgende Algorithmus verwendet werden:
\begin{enumerate}
    \item Starte mit Intervall \([l, r] \leftarrow [1, n]\), und \(F = \) dem neutralen Element von \(f\).

    \item Überlappt \([l, r]\) vollständig mit \([i,j]\)?
        Dann \(F \leftarrow f(F, f(a[l, r]))\).

    \item Überlappt \([l, r]\) überhaupt nicht mit \([i,j]\)?
        Dann lasse \(F\) gleich.

    \item Überlappt \([l, r]\) teilweise mit \([i,j]\)?
        Die Mitte ist \(\lfloor(l+r)/2\rfloor\).
        Mache mit \([l, m]\) und \([m+1, r]\) rekursiv weiter.
\end{enumerate}
Insgesamt müssen also nur \(\O(\log n)\) Einträge ausgewertet werden.
Das Point-Update funktioniert genau wie bei den Fenwick-Bäumen -- ein Update wird den jeweiligen Elternknoten weitergegeben.

Das Range Update würde naiv im worst case \(2n-1\) Updates nötig machen, weswegen sogenannte \emph{Lazy Updates} meistens verwendet werden, die Updates erst ausführen, wenn sie bei einer Range-Query relevant wären.
Man kann sowas implementieren, indem man die Updates in den Blattknoten vormerkt, und dann bei der Auswertung einer Query, alle relevanten Updates backpropagiert.
Somit bleibt man bei \(\O(\log n)\) bei einer Range Query, und ein Range Update kann mit minimalen Overhead \(\O(\log n)\) realisiert werden.
